...
export default function Events() {
  ...
  const fetchEvents = async () => {
    try {
      const res = await axios.get("https://backenderp-production-6374.up.railway.app/api/institute/events/all");
      setEvents(res.data);
      console.log(res.data);
    } catch (err) {
  ...
  const handleAdd = async (e) => {
    e.preventDefault();
    try {
      await axios.post("https://backenderp-production-6374.up.railway.app/api/institute/events/create", form);
